...
def bbands(series: pd.Series, length: int = 20, std_mult: float = 2.0) -> pd.DataFrame:
...
def adx_dmi(high: pd.Series, low: pd.Series, close: pd.Series, length: int = 14) -> pd.DataFrame:
    up_move = high.diff()
    down_move = -low.diff()
...
def obtener_datos_binance(symbol: str):
    data = {
...
def contar_toques(df: pd.DataFrame, nivel: float, atr: float, factor: float = 0.15) -> int:
    if atr <= 0:
        return 0
...
def agrupar_zonas(niveles: List[float], tolerancia_pct: float = 0.0008) -> List[Tuple[float, float]]:
    if len(niveles) == 0:
        return []
...
def construir_liquidity_engine(df_1m: pd.DataFrame, atr: float, price: float) -> Dict:
...
def calcular_posicion_real(entrada: float, sl: float, tp: float) -> Optional[Dict]:
...
# LOGGING / BACKTEST
# ============================================================
def log_signal(tipo: str, prob: float, fase: str, adx: float, rsi_1m: float) -> None:
    row = {
        "time": datetime.now(),
        "tipo": tipo,
        "fase": fase,
...
def condiciones_similares(df: pd.DataFrame, direction: str) -> List[int]:
    condiciones = []
...
def probabilidad_historica(df: pd.DataFrame, direction: str) -> float:
...
def backtest(df: pd.DataFrame, direction: str = "long") -> Tuple[int, int, float, float]:
    wins = 0
    losses = 0
    capital = 100.0
